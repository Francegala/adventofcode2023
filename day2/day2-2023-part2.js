...
for (i = 0; i < games.length; i++) {
  ...
  let game = games[i].split(/^Game \d+: /)[1];
  let red = game.match(/\d+ red/g);
  red.forEach((entry) => {
    let regex = entry.match(/\d*/)[0]
    if (Number.parseInt(regex) > Number.parseInt(redMax)) {
      redMax = Number.parseInt(regex);
    }
  });
  let green = game.match(/\d+ green/g);
  green.forEach((entry) => {
    let regex = entry.match(/\d*/)[0]
    if (Number.parseInt(regex) > Number.parseInt(greenMax)) {
      greenMax = Number.parseInt(regex);
    }
  });
  let blue = game.match(/\d+ blue/g);
  blue.forEach((entry) => {
    let regex = entry.match(/\d*/)[0]
    if (Number.parseInt(regex) > Number.parseInt(blueMax)) {
      blueMax = Number.parseInt(regex);
    }
  });
