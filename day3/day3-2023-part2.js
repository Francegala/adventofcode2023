...
function findWholeNumber(row, index) {
  let number = "";
  let left = index;
  while (left >= 0 && !isNaN(row[left]) && row[left] !== ".") {
...
for (i = 0; i < engine.length; i++) {
  const row = engine[i].split("");
  let result = [];
  row.forEach((value, index) => {
    if (value == "*") {
      const rowUP = engine[i - 1].split("");
      const rowDown = engine[i + 1].split("");

      let val1 = ["", 0];
      let val2 = ["", 0];
      let val3 = ["", 0];
      for (j = 0; j < 3; j++) {
        val1 = [findWholeNumber(rowUP, index - 1 + j)[0], j];
        if (val1[0] != "") {
          if (result.length < 1) {
            result.push([val1[0], i, index, val1[1], "up"]);
            val1 = ["", 0];
          } else if (val1[1] != result[result.length - 1][3]) {
            result.push([val1[0], i, index, val1[1], "up"]);
            val1 = ["", 0];
          }
        }

        val2 = [findWholeNumber(row, index - 1 + j)[0], j];
        if (val2[0] != "") {
          if (result.length < 1) {
            result.push([val2[0], i, index, val2[1], "row"]);
            val2 = ["", 0];
          } else if (val2[1] != result[result.length - 1][3]) {
            result.push([val2[0], i, index, val2[1], "row"]);
            val2 = ["", 0];
          }
        }

        val3 = [findWholeNumber(rowDown, index - 1 + j)[0], j];
        if (val3[0] != "") {
          if (result.length < 1) {
            result.push([val3[0], i, index, val3[1], "down"]);
            val3 = ["", 0];
          } else if (val3[1] != result[result.length - 1][3]) {
            result.push([val3[0], i, index, val3[1], "down"]);
            val3 = ["", 0];
          }
        }
  ...
  if (result.length > 1) {
    parts += Number.parseInt(result[0][0]) * Number.parseInt(result[1][0]);
  }
}
