...
for (i = 0; i < engine.length; i++) {
  let numbers = engine[i].match(/\d+/g);
  if (numbers != null)
    for (j = 0; j < numbers.length; j++) {
      let index = engine[i].indexOf(numbers[j]);
      let length = numbers[j].length;
      let symbol = engine[i - 1]
        .substring(index - 1, index + length + 1)
        .replace(/\d/g, "")
        .replace(/\./g, "");
      let symbol2 = engine[i]
        .substring(index - 1, index + length + 1)
        .replace(/\d/g, "")
        .replace(/\./g, "");
      let symbol3 = engine[i + 1]
        .substring(index - 1, index + length + 1)
        .replace(/\d/g, "")
        .replace(/\./g, "");
      if (symbol.length > 0) parts += Number.parseInt(numbers[j]);
      if (symbol2.length > 0) parts += Number.parseInt(numbers[j]);
      if (symbol3.length > 0) parts += Number.parseInt(numbers[j]);
    }
}
