...
let map = new Map();
values.map((value) => {
  map.set(value[0], value[1].substring(1, value[1].length - 1).split(', '));
});
let keyA = Array.from(map.keys()).filter((key) => /(A)$/.test(key));
let keyZ = Array.from(map.keys()).filter((key) => /(Z)$/.test(key));
console.log(keyA);
let KLA = keyA[0];
let AAA = keyA[1];
let NDA = keyA[2];
...
let NNA = keyA[4];
let QVA = keyA[5];
console.log(KLA, AAA, NDA, LBA, NNA, QVA);
console.log(keyZ);
...
while (
  ...
  let nodeKLA = map.get(KLA);
  let nodeAAA = map.get(AAA);
  let nodeNDA = map.get(NDA);
  let nodeLBA = map.get(LBA);
  let nodeNNA = map.get(NNA);
  let nodeQVA = map.get(QVA);

  let direction = commandsArray[i];
  if (direction === 'R') {
    KLA = nodeKLA[1];
    AAA = nodeAAA[1];
    NDA = nodeNDA[1];
